...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const handleDeviceOrientation = useCallback((event: DeviceOrientationEvent): void => {
    const shell = shellRef.current;
    if (!shell || !tiltEngine) return;
  ...
  useEffect(() => {
    ...
    const centerX = (shell.clientWidth || 0) / 2;
    const centerY = (shell.clientHeight || 0) / 2;
        ...
        >
          ...
          >
            ...
            >
              {avatarUrl ? (
                <img
                  className="absolute inset-0 w-full h-full object-cover object-center will-change-transform transition-transform duration-[120ms] ease-out"
                  src={avatarUrl}
                  ...
                  style={{
                    ...
                    transform:
                      'translate3d(calc((var(--pointer-from-left) - 0.5) * 8px), calc((var(--pointer-from-top) - 0.5) * 8px), 0)',
                    borderRadius: cardRadius,
                    backfaceVisibility: 'hidden'
                  }}
                />
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-white/85">
                  <div className="w-24 h-24 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-4xl font-bold">
                    {(name || 'U').charAt(0).toUpperCase()}
                  </div>
                  <span className="text-xs text-white/60 tracking-wide">No profile photo</span>
                </div>
              )}
            ...
            >
              <div className="w-full absolute flex flex-col top-12">
                <h3
                  className="font-semibold m-0"
                  style={{
                    fontSize: 'min(4.2vh, 2.3em)',
