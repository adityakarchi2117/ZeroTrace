...
export const verificationAPI = {
  ...
  getVerificationSummary: async (userId: number): Promise<VerificationSummary> => {
    const response = await api.http.get(`/api/verification/summary/${userId}`);
  ...
  getMyBadges: async (): Promise<VerificationBadge[]> => {
    const response = await api.http.get('/api/verification/badges');
    return response.data;
  },
  ...
  getUserBadges: async (userId: number): Promise<VerificationBadge[]> => {
    const response = await api.http.get(`/api/verification/badges/${userId}`);
    return response.data;
  },
  ...
  createVerificationRequest: async (
    ...
    const response = await api.http.post('/api/verification/request', payload);
    return response.data;
  },
  ...
  getMyRequests: async (): Promise<VerificationRequest[]> => {
    const response = await api.http.get('/api/verification/requests');
    return response.data;
  },
  ...
  getPendingRequests: async (): Promise<VerificationRequest[]> => {
    const response = await api.http.get('/api/verification/requests/pending');
    return response.data;
  },
  ...
  reviewRequest: async (payload: ReviewVerificationPayload): Promise<VerificationRequest> => {
    const response = await api.http.post('/api/verification/review', payload);
    return response.data;
  },
  ...
  getVerificationHistory: async (): Promise<VerificationHistoryEntry[]> => {
    const response = await api.http.get('/api/verification/history');
    return response.data;
  },
  ...
  revokeBadge: async (verificationType: VerificationType): Promise<{ message: string }> => {
    const response = await api.http.delete(`/api/verification/badge/${verificationType}`);
    return response.data;
  },
};
 ...
 */
export function getTypeDescription(type: VerificationType): string {
